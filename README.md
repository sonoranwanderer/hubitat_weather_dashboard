...
4. Upload `dist/weather-dashboard.js` (generated by the build step described below) to Hubitat's **File Manager** so it is available at `/local/weather-dashboard.js`. Files stored in File Manager are served under the `/local/` path used by dashboards and drivers.
...
npm run build         # outputs dist/weather-dashboard.js
```
