...
4. Upload the dashboard assets to Hubitat's **File Manager** so they are served from the `/local/` path that dashboards and apps expect:
   * `dashboard/weather-dashboard.js` (generated by the build step described below)
   * `app/weather-dashboard-app.js`
   * `app/weather-dashboard-app.html`
   Keeping the files in the File Manager root publishes them at `/local/<filename>`, which lets the in-app preview work without additional configuration.
