...
const preludePath = path.join(projectRoot, 'src', 'bootstrap', 'hubitat-prelude.js');
...
function loadPrelude() {
  try {
    ...
    console.warn('[esbuild] Unable to load Hubitat prelude:', err?.message || err);
    return '';
  }
}
...
function createBuildOptions({ minify = false, sourcemap = false, outfile = outFile, logLevel = 'info' } = {}) {
  return {
    ...
    logLevel,
    banner: { js: [bannerComment(), loadPrelude()].filter(Boolean).join('\n') },
    define: {
