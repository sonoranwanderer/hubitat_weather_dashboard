...
function assertCssContracts() {
  ...
  assert(source.includes("px('--wdash-rain-composition-width-fluid', 640, 960);"), 'rain card should expose a bounded composition width');
  assert(source.includes('.wdash-rain-main { display: grid; grid-template-columns: minmax(0, var(--wdash-rain-drop-track-fluid, 132px)) minmax(0, var(--wdash-rain-primary-track-fluid, 210px)) minmax(0, var(--wdash-rain-stats-track-fluid, 240px)); gap: var(--wdash-rain-gap-fluid, 16px);'), 'rain grid should use bounded drop, primary, and stats tracks');
  assert(source.includes('width: min(100%, var(--wdash-rain-composition-width-fluid, 640px));'), 'rain grid should cap the internal composition instead of stretching columns across excess width');
  assert(source.includes('margin-inline: auto;'), 'rain grid should balance outer gutters around the internal composition');
  assert(!source.includes('--wdash-rain-edge-gutter-fluid'), 'rain grid should not use artificial side padding to compensate for a full-width composition');
